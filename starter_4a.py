...
train_dataset = IddDataset_4a(csv_file='train.csv', aug = False)
train_dataset_aug = IddDataset_4a(csv_file='train.csv', aug = True)
train_dataset = torch.utils.data.ConcatDataset([train_dataset, train_dataset_aug])                                
val_dataset = IddDataset_4a(csv_file='val.csv', aug = False)
test_dataset = IddDataset_4a(csv_file='test.csv', aug = False)
...
def val(epoch):
    ...
    with torch.no_grad():
        for iter, (X, tar, Y) in enumerate(val_loader):
            ...
            accuracy_mini_batch.append(accuracy_)
            iou_mini_batch.append(avg_iou)
            ious_total = [sum(x) for x in zip(ious_total, ious)]
